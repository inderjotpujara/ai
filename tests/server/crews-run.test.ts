...
function tempStore() {
  return createJobStore(
    { path: mkdtempSync(join(tmpdir(), 'crewrun-jobs-')) },
    {},
  );
}
...
test('200 + {runId}, pre-creates dir, ENQUEUES a crew job (does not run inline)', async () => {
  ...
  expect(items[0]?.kind).toBe(JobKind.Crew);
  expect(items[0]?.status).toBe(JobStatus.Queued);
  expect(items[0]?.runId).toBe(runId); // job.runId === run dir id
  expect(items[0]?.payload).toEqual({ name: 'research-crew', input: 'AI' });
  jobStore.close();
});

test('unknown crew → 404 (no dir created, nothing enqueued)', async () => {
  const jobStore = tempStore();
  const res = await handleCrewRun(
    runReq('nope', { input: 'x' }),
    { runsRoot: root, jobStore },
