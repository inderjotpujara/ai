...
test('allows an absent Origin (same-origin nav) and a listed origin; rejects cross-origin', () => {
  ...
  expect(
    originAllowed(
      req({ host: 'localhost:4130', origin: 'http://localhost:4130' }),
      policy,
    ),
  ).toBe(true);
  expect(
    originAllowed(
      req({ host: 'localhost:4130', origin: 'https://evil.example.com' }),
      policy,
    ),
  ).toBe(false);
});
...
test('rejects a bare (portless) loopback Origin when not explicitly allowlisted', () => {
  ...
  expect(
    originAllowed(
      req({ host: 'localhost:4130', origin: 'http://localhost' }),
      noBareAllowlist,
    ),
  ).toBe(false);
});
...
test('accepts a distinct non-loopback origin configured via allowedOrigins', () => {
  const tunnelPolicy: OriginPolicy = {
    port: 4130,
    allowedOrigins: ['https://tunnel.example.com'],
  };
  expect(
    originAllowed(
      req({ host: 'localhost:4130', origin: 'https://tunnel.example.com' }),
      tunnelPolicy,
    ),
  ).toBe(true);
});
