...
import {
  OllamaPullAggregator,
  parseOllamaLine,
} from '../../src/provisioning/ollama-pull.ts';
...
describe('parseOllamaLine', () => {
  it('maps "pulling manifest" to Resolving', () => {
    expect(parseOllamaLine('{"status":"pulling manifest"}')?.phase).toBe(
  ...
  it('treats presence of digest+total+completed as Downloading regardless of verb', () => {
    const r = parseOllamaLine(
      '{"status":"pulling 12ab","digest":"sha256:12ab","total":100,"completed":40}',
    );
  ...
  it('maps "verifying sha256 digest" to Verifying', () => {
    expect(parseOllamaLine('{"status":"verifying sha256 digest"}')?.phase).toBe(
      DownloadPhase.Verifying,
    );
  });
  it('maps "success" to Done', () => {
    expect(parseOllamaLine('{"status":"success"}')?.phase).toBe(
      DownloadPhase.Done,
    );
  });
