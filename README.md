...
> default) behind durable per-device auth. Next: **Slice 25** (scheduled /
> triggered agents, enqueuing onto this queue). See
...
| **Next (product line)** | Toward a local **n8n × CrewAI**: **Slice 30b is fully shipped** (all 8 phases — dictation-only voice; barge-in remains explicit future scope, not debt); **Slice 23** (AI SDK v6→v7 upgrade) ✅ **shipped** (2026-07-19), clearing the dependency-line hold; **Slice 24** (always-on daemon + task queue + resumable jobs + secure remote access) ✅ **shipped** — Phase E's always-on identity begins; **next: Slice 25** (scheduled/triggered agents, cron/webhook/event triggers enqueuing onto the Slice-24 queue); Codex heavy-lifting backup (Slice 22) deferred to the very end (Slice 38) | Planned |
