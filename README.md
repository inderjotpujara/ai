...
> a new Ops-console Federation tab. **Slice 32 (self-improvement loop) has
> since shipped** — the first slice past Phase E: `src/self-improve/`
> continuously **re-evaluates a generated artifact's persisted golden set
> whenever the model underneath it drifts** (a Cron sweep + a hook off every
> `model.pull`, both riding the existing trigger substrate onto a new `Eval`
> job kind), and on a noise-robust confirmed regression **auto-demotes**
> (`Behaves→Unverified`) and surfaces it — closing the "verified once at
> creation, never re-checked" gap without ever regenerating. See
...
> §"`src/a2a/` — A2A interop".
>
> **Slice 32 (self-improvement loop — continuous re-eval on model swap) — now
> also ✅ shipped**: the verified-build gate (Slice 20) proved an artifact
> **once, at creation**; nothing re-checked it as models were pulled or the
> catalog shifted. `src/self-improve/` closes that — it **replays an
> artifact's persisted golden set whenever the model under it drifts**, and on
> a confirmed regression **demotes** (`Behaves→Unverified`) and surfaces it. It
> is *generation-free* (never regenerates/repairs — demote + surface only).
> **Baseline:** the gate now records `ManifestEntry.verifiedWith` (the resolved
> model identity, `MANIFEST_VERSION` 1→2). **Detection:** two repo triggers
> (`triggers/index.ts`) ride the existing Slice-25 substrate with **no new
> `TriggerType`** — a Cron `reeval-sweep` (`0 4 * * *`) + a JobChain
> `reeval-on-pull` (fires after every completed `model.pull`) — both enqueuing a
> new **`JobKind.Eval`** job onto the Slice-24 queue. **Decision (D4):** a
> **pure** `decideRegression` — per-case regressed set + bounded unanimous-fail
> re-run + a hysteresis band (`AGENT_REEVAL_HYSTERESIS`, default `0.15`) — so
> judge noise can't trip a false demote; only a confirmed `Regression`
> demotes. **Action (D5):** append an append-only `eval_history` row (in the
> shared `jobs.db`) on every verdict; on a confirmed regression also demote
> (idempotent), record a `ModelDegraded` degrade, and emit an `eval.regression`
> span. Surfaced via `GET/POST /api/evals*`, a new Ops-console **Evals/Health**
> tab, `bun run reeval [--all | --agent <name>]`, and `eval.*` telemetry. Four
> `AGENT_REEVAL_*` knobs (master switch default-on). **Honest caveats:**
> crew/workflow drift re-eval resolves against a **default** requirement (not
> each member's own model pick); the 👎 `chat.feedback` signal is read as an
> **unattributed count only** (no messageId→artifact join yet — a documented
> follow-on); a **quant-only** model swap may not trip the tag-level drift diff.
> Live-verify against a real Ollama model swap is pending at time of writing.
> See [`docs/architecture.md`](docs/architecture.md) §"`src/self-improve/` —
> continuous re-eval loop".
> **Phase 2**
...
| **31** | **Multi-machine + A2A interop** (Phase E, web-focused — closes Phase E) — one hand-rolled **A2A v1.0** layer (JSON-RPC over HTTP+SSE; not `@a2a-js/sdk`, still v1.0-beta) over the Slice-24 daemon/queue, driven from a new Ops-console **Federation tab**. **EXPOSE:** `GET /.well-known/agent-card.json` (public discovery, outside the `/api` guard, `404` fail-safe while `AGENT_A2A_ENABLED` is off — default off) advertises skills 1:1 from a least-privilege allowlist (`src/a2a/allowlist.ts` — only registered Chat/Crew/Workflow refs, author-time **and** invoke-time resolve-then-reject); `POST /api/a2a` (JSON-RPC `message/send`/`tasks/get`/`tasks/cancel`) verifies a SEPARATE A2A-Bearer credential (D5, distinct token domain from a browser device session, sharing only the root HMAC key) **before the body is even parsed**, then a replay guard (nonce+timestamp), then maps the (untrusted-fenced) inbound task onto the **same** `JobStore.enqueue` (`origin=Remote`, `taskId===jobId`); `message/stream`/`tasks/resubscribe` re-frame the **existing** run-stream SSE engine (no parallel stream) into A2A `TaskStatusUpdateEvent`/`TaskArtifactUpdateEvent` frames. **CONSUME:** `src/a2a/client.ts` discovers + hash-pins a remote's card (SSRF-guarded via the existing `redirect:'error'` fetch, timeout+size-capped, a hash mismatch is a HARD reject — never a silent re-pin) and `src/a2a/mount.ts` shapes a `delegate_to_<name>` `ToolSet` matching the MCP mount's failure-returns-not-throws contract (send→poll-to-terminal, per-remote circuit breaker) — **spliced into a live CHAT session's own tool set** via `liveRemoteDelegateTools` → `createOrchestrator` (both web + CLI chat entrypoints, gated by `AGENT_A2A_ENABLED`, local agent wins on a name collision), so a console/CLI-added remote is a real mid-CHAT-turn delegate target, no restart; **crew/workflow consume-delegation remains a documented seam-ready follow-on** (`mountAll`/`loadMcpConfig` still don't read the remote store — crew/workflow build their toolset via a different seam); `agent a2a call` and the Federation tab's recent-remote-tasks history (reusing the existing Runs browser, zero new run-history plumbing) remain for crew/workflow and direct manual invocation. New `agent a2a skills|token|remotes|call|card` CLI mirroring `agent daemon`'s shape. Adversarial review caught + fixed **three real defects**: a non-exposable `JobKind` could slip past the allowlist's least-privilege check, a terminal SSE frame could arrive before a same-poll child's progress frame (data loss on a spec-conformant client), and the consume-side delegate loop originally returned `message/send`'s `submitted` shell as if it were the answer (every real delegation would have failed). See [`docs/architecture.md`](docs/architecture.md) §"`src/a2a/` — A2A interop" | ✅ Done |
| **32** | **Self-improvement loop — continuous re-eval on model swap** (first slice past Phase E) — closes the "verified once at creation, never re-checked" gap (Slice 20's gate proved an artifact against the model resolved *then*; nothing re-checked it as models drifted). `src/self-improve/` **replays a generated artifact's persisted golden set whenever the model under it changes** and, on a noise-robust confirmed regression, **auto-demotes** (`Behaves→Unverified`) + surfaces it — *generation-free* (never regenerates/repairs). **Baseline:** the verified-build gate now records `ManifestEntry.verifiedWith` (resolved model identity, `MANIFEST_VERSION` 1→2, via `verified-with.ts`); the shared `runGoldenEval` binding is extracted from `verified-build/eval.ts` so the gate and the re-eval engine share one eval path. **Detection:** two repo `TriggerDef`s (`triggers/index.ts`) ride the existing Cron + JobChain types with **no new `TriggerType`** — `reeval-sweep` (Cron, `reevalSweepCron()` default `0 4 * * *`, hot-first, seeds missing baselines, R4 de-dup) + `reeval-on-pull` (JobChain off every `model.pull` Done, coalesced) — enqueuing a new **`JobKind.Eval`** (`EvalMode {Sweep\|AffectedByPull\|Artifact}`) onto the Slice-24 queue. **Replay:** `reeval.ts`/`replay-case.ts` re-run the golden set MCP-free across agent/crew/workflow. **Decision (D4, `regression.ts`):** a **pure** `decideRegression` — per-case regressed set + bounded unanimous-fail re-run (`AGENT_REEVAL_RERUN_CASES`, default 2) + a hysteresis band (`AGENT_REEVAL_HYSTERESIS`, default 0.15, strict `>`) → `Pass`/`Regression`/`WithinNoise`/`Inconclusive`; only `Regression` demotes. **Action (D5, `action.ts`):** always append an append-only `eval_history` row (in the shared `jobs.db` via the `JOBS_DB_MIGRATIONS` superset); on a confirmed regression also demote (idempotent), `recordDegrade(ModelDegraded)`, and emit `eval.regression`. Surfaced via `GET /api/evals`, `GET /api/evals/:artifact`, `POST /api/evals/reeval` (trusted-local), a new Ops **Evals/Health** tab, `bun run reeval [--all\|--agent <name>]`, `eval.*` spans, and four `AGENT_REEVAL_*` knobs (master switch default-on; a manual artifact eval runs even when disabled). Honest caveats: crew/workflow drift resolves against a **default** requirement, not each member's model; the 👎 `chat.feedback` signal is an **unattributed count only** (no messageId→artifact join yet); a quant-only swap may not trip drift. See [`docs/architecture.md`](docs/architecture.md) §"`src/self-improve/` — continuous re-eval loop" | ✅ Done |
| **Next (product line)** | Toward a local **n8n × CrewAI**: **Slice 30b is fully shipped** (all 8 phases — dictation-only voice; barge-in remains explicit future scope, not debt); **Slice 23** (AI SDK v6→v7 upgrade) ✅ **shipped** (2026-07-19), clearing the dependency-line hold; **Slice 24** (always-on daemon + task queue + resumable jobs + secure remote access) ✅ **shipped**; **Slice 25b** (Jobs & Triggers Ops Console) ✅ **shipped**; **Slice 25** (the triggers backend — cron/webhook/file-watch/job-chain enqueuing onto the Slice-24 queue, live-wiring the console's Triggers tab) ✅ **shipped**; **Slice 31** (multi-machine + A2A interop) ✅ **shipped** — **Phase E complete**; **Slice 32** (self-improvement loop — continuous re-eval on model swap) ✅ **shipped**; **next: the remaining 33–37 backlog** (portability/sharing, scale-beyond-one-crew, product surface, security hardening, CodeAct — see [`docs/ROADMAP.md`](docs/ROADMAP.md)'s recommended sequence); Codex heavy-lifting backup stays deferred to the very end (Slice 39) | Planned |
