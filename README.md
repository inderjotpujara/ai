...
> remaining open item is the routing-accuracy eval harness.) Next: **Slice 29
> (re-scoped 2026-07-07)** — voice **input** via a mature OSS STT library
> (`sherpa-onnx`, CLI + web). Its original "voice in/out + streaming CLI" scope
> was built and **reset** (a hand-rolled terminal voice pipeline that fought the
> OS; archived on branch `slice-29-voice-streaming-cli`) — rich interruptible
> voice now lives in the web-UI slice, on browser-native echo cancellation.
> **Slices 23/24/25 remain held** on the `ai@7` provider blocker. See
> [`docs/ROADMAP.md`](docs/ROADMAP.md).

> **Status:** Slice 28 complete — **Hardware-adaptive media generation +
...
| **Next (product line)** | Toward a local **n8n × CrewAI**: **F** voice **input** via an OSS STT library (`sherpa-onnx`, CLI + web — **Slice 29, re-scoped** from the reset "voice in/out + streaming CLI"; that hand-rolled pipeline is archived on branch `slice-29-voice-streaming-cli`) → TUI/local web UI (rich interruptible voice lives here, on browser-native AEC); **E** automate (always-on daemon + secure remote access, **Slice 24**; scheduled/triggered agents, 25) — held on the `ai@7` provider blocker (Slices 23/24/25); Codex heavy-lifting backup (Slice 22) deferred to the very end (Slice 38) | Planned |
