...
> break-glass root rotate, with the Triggers screen designed but stubbed.
> Next: **Slice 25** (the triggers *backend* — scheduled/triggered agents,
> cron/webhook/event enqueuing onto this queue). See
...
> write check; the Triggers tab is designed but stubbed until Slice 25's
...
| **Next (product line)** | Toward a local **n8n × CrewAI**: **Slice 30b is fully shipped** (all 8 phases — dictation-only voice; barge-in remains explicit future scope, not debt); **Slice 23** (AI SDK v6→v7 upgrade) ✅ **shipped** (2026-07-19), clearing the dependency-line hold; **Slice 24** (always-on daemon + task queue + resumable jobs + secure remote access) ✅ **shipped** — Phase E's always-on identity begins; **Slice 25b** (Jobs & Triggers Ops Console — the web UI for Slice 24's daemon/queue/auth, plus device pairing/revoke + break-glass root rotate) ✅ **shipped**; **next: Slice 25** (the triggers *backend* — cron/webhook/event enqueuing onto the Slice-24 queue behind the console's already-designed, currently-stubbed Triggers tab); Codex heavy-lifting backup (Slice 22) deferred to the very end (Slice 38) | Planned |
