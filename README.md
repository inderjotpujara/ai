...
> **Slice 30** (TUI / local web UI), on browser-native echo cancellation.
> Next: **Slice 30** — the TUI / local web UI. **Slices 23/24/25 remain held**
> on the `ai@7` provider blocker. See [`docs/ROADMAP.md`](docs/ROADMAP.md).

> **Status:** Slice 29 complete — **CLI voice input (STT)**: `src/voice/`
...
> so Slice 30's web UI can transcribe without a second STT stack. Mic
...
> barge-in, and true hold-to-talk stay out of scope — Slice 30's browser UI
> owns rich/interruptible voice via real `getUserMedia` AEC. See
> [`docs/architecture.md`](docs/architecture.md) §23.

> **Previously:** Slice 28 — **Hardware-adaptive media generation +
