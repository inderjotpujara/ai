...
- **The SDD ledger (`.superpowers/sdd/progress.md`) is hard-lined too — same bar, same gate.** Every slice appends its per-task / review / fix / landing entries to the ledger *as it progresses*, and the **pre-push slice-landing gate blocks a slice from landing on main unless the ledger is updated in the same push** — exactly like README + ROADMAP. It is the continuity record a fresh session (or another machine) resumes from, so a shipped slice with a stale ledger is a defect. (The `.remember/` continuity buffers are **hook-managed/auto** — refreshed by the `remember` skill + the SessionStart/PreCompact hooks — so they are *not* gated; only the deliberate SDD ledger is.)
...
- **pre-push** → (a) blocks a push whose commits change `src/**` but not `docs/architecture.md`; (b) **slice-landing gate** — a push **to main** that changes `docs/architecture.md` (= a slice shipped) is blocked unless `README.md`, `docs/ROADMAP.md` **and** `.superpowers/sdd/progress.md` (the SDD ledger) are all updated in the same push (and it reminds you to regenerate the Artifact). Deliberate bypass only (genuinely not a slice, e.g. a typo): `DOCS_OK=1 git push`.
