...
- **EXPOSE.** The daemon serves an Agent Card at `GET /.well-known/agent-card.json` and a JSON-RPC endpoint at `POST /api/a2a`. `message/send` maps an inbound A2A task onto the **existing Slice-24 queue** (`JobStore.enqueue`) and returns a `Task` in `submitted`; the worker flips it `working → completed/failed`; `message/stream` re-frames the existing run-stream machinery as A2A `TaskStatusUpdate`/`TaskArtifactUpdate` SSE events. `OrchestratorResult` (variants `answer|gap|resource` only — `src/core/orchestrator.ts:21`; there is **no** `consent` variant) maps to task states (answer→completed, gap/resource→failed). Consent is a **mid-run event, not a result variant**: a remote task runs as a queued job whose dispatch hardcodes consent **fail-closed** (`confirm: async () => false`, `src/server/jobs/dispatch.ts:200`), so a mid-run consent gate lands as a typed **`failed`** (`consent-unavailable`) — matching the existing queued-job posture, never a hang.
...
- **No interactive mid-run consent for queued jobs (A2A `input-required` round-trip)** — a remote A2A task runs as a queued job, and the queue's dispatch already runs consent **fail-closed** (`confirm: async () => false`, `src/server/jobs/dispatch.ts:200`) with no live client, a `promptId`-keyed (not `runId`-keyed) consent seam, and consent prompts as `StatusEvent`s the A2A stream framer doesn't see. So a mid-run consent gate lands as a typed `failed` (`consent-unavailable`), matching the queued-job posture (§7.1). `TaskStateWire.input-required`/`auth-required` stay in the enum for protocol completeness but are **not emitted** this slice. A durable **queue-consent capability** — a `promptId`-carrying `ConfirmPort`/`EventSink` injected into dispatch so a queued job (A2A *or* local) can round-trip a consent prompt and resume — is a **scoped future slice**, not silent debt.
...
- **`src/a2a/task-map.ts`** — the `OrchestratorResult`/`JobStatus` ↔ A2A `Task`-state bijection incl. the fail-closed mid-run-consent → typed `failed` (`consent-unavailable`) projection (§7.1).
...
- **Task-state mapping (`task-map.ts`), JSON-RPC lowercase-hyphenated casing:**
  ...
  | consent needed mid-run | `failed` (typed `consent-unavailable`) | **fail-closed** — the queued-job dispatch runs `confirm: async () => false`; `input-required` is NOT emitted this slice (§7.1) |
...
4. **Mount** through the existing MCP mount path: `client.ts` exposes the remote's `skills[]` as a `ToolSet` (one tool per skill, `execute` = `message/send`/`message/stream` against the remote) and registers it via `mountAll` (`src/mcp/mount.ts:98`) → `MountedRegistry.forAgent(name)` (`mount.ts:204`) → scoped into `createSuperAgent` `toolsFor` (`agents/super.ts:26`). The mounted remote therefore becomes a `delegate_to_<name>` specialist that **inherits the guardrails, depth-guard, breaker, and `agent.delegation` span** for free (`src/core/delegate.ts:122`, `runGuardedAgent`). Consent + hash-pinning reuse `mountAll`'s existing consent flow.
...
4. **EXPOSE streaming** — `message/stream` / `tasks/resubscribe` re-framing the run-stream as `TaskStatusUpdate`/`TaskArtifactUpdate`; fail-closed mid-run consent → typed `failed` (`consent-unavailable`), no hang (§7.1).
...
- **7.1 `OrchestratorResult`/`JobStatus` ↔ A2A task-state mapping (totality) + fail-closed mid-run consent (no hang).** Two correctness obligations, no consent round-trip. **(a) State-mapping totality:** every `OrchestratorResult` variant (`answer|gap|resource` — `src/core/orchestrator.ts:21`) and every `JobStatus` maps to exactly one A2A task state, with no default-to-`completed` hole and `gap`/`resource` never projecting to `completed`. **(b) Fail-closed consent:** a remote A2A task runs as a queued job whose dispatch already hardcodes consent fail-closed (`confirm: async () => false`, `src/server/jobs/dispatch.ts:200`; `events: noopEventSink`). There is no live client, the consent seam is keyed by `promptId` (not `runId`), and a consent prompt is a `StatusEvent` (not the `SpanDTO` the stream framer sees) — so there is **no substrate for a cross-machine consent round-trip**. A mid-run consent gate therefore lands the job `Failed` → the task surfaces a typed **`failed`** (`consent-unavailable`) and reaches a terminal state within the run's wall-clock — **never a hang, never `input-required`**. `input-required`/`auth-required` remain in `TaskStateWire` for protocol completeness but are not emitted this slice.
...
- **EXPOSE server (mocked queue).** `message/send` → `submitted` + a job enqueued with the mapped `JobKind`; each `OrchestratorResult` variant → the correct task state (answer/gap/resource); a job that hits a mid-run consent gate → typed `failed` (`consent-unavailable`), deterministically, never a hang; `tasks/cancel` fires the AbortSignal; unlisted skill → reject (7.4); bad/absent Bearer → 401 before parse (7.2); over-cap body → 413; replayed request → rejected.
- **Streaming (fake stream).** `message/stream` emits `TaskStatusUpdate` submitted→working→completed and a `TaskArtifactUpdate`; `tasks/resubscribe` replays by `Last-Event-ID`; a run that hits a mid-run consent gate terminates as a typed `failed` (`consent-unavailable`) within the wall-clock — `input-required` never emitted (7.1).
