...
- **Video *generation* live-verify is deferred, not silently skipped** — same posture as Slice 14's non-Ollama runtimes. `mlx-video`'s real `ltx_2`/`wan_2` implementation exists only in the `Blaizzy/mlx-video` git repo (PyPI ships a 5.1 kB stub); it depends on `mlx_vlm`, whose current release (0.6.3) hard-conflicts with `transformers>=5` — which the already-live-verified mflux/mlx-audio installs require in the same venv, so downgrading to unblock video would break image/speech generation. Unblocks via an isolated venv for `AGENT_VIDEO_CMD` (the env-configurable-command design already supports this) or `mlx-video` ecosystem maturity. The generation code (`ltxStrategy`, `generate_video` tool, server-lane degrade dispatcher) is complete, unit-tested, and reviewed.
...
- **Kokoro TTS needs `misaki[en]` installed** (a G2P dependency, not bundled) and the image-gen default needs an ungated FLUX-schnell mirror (or an HF token + accepted license for the gated `black-forest-labs/FLUX.1-schnell` itself) — both are provisioning notes, not code gaps; neither is auto-installed by this framework today.
