...
- **GitHub remote-HTTP live-verify** — the `github` pack entry (Streamable HTTP + PAT header) shipped contract-tested; live-verify deferred until a `GITHUB_PAT` is available (recorded in architecture.md §14 + SDD ledger).
- **Interactive consent-prompt UX live-verify** — headless consent (`AGENT_MCP_AUTO_APPROVE=1`) live-verified; the interactive TTY y/N prompt path is unit-tested but awaits a first real terminal run.
