...
`.generated.json` manifest; ≥0.85 reuse · 0.75–0.85 offer · <0.75 generate),
...
real runtime error back), a **golden-eval** (3–7 auto-generated binary cases,
judged unanimously 3× by the largest installed cross-family model — skipped
...
9e. ✅ **shipped, Slice 20** — **Verified "works out of the box"** ⭐ (Phase D — closes the phase). `src/verified-build/` turns both builders' write step into **stage → verify → commit** through one shared, cheapest-first gate (`gate.ts` `verifyAndCommit`, serving all three `ArtifactKind`s): a **pre-generation reuse check** (a `CapabilitySignature` of the need is embedded via the Slice-12 embedder and cosine-compared against a per-registry `.generated.json` **manifest sidecar** — ≥0.85 reuse the existing artifact, 0.75–0.85 offer it, <0.75 generate) → **stage** to a def file only (the builders' `write.ts` modules split into stage/register halves; never the registry index) → **structural** re-check via the existing validators → **execution dry-run** against a benign read-only representative task (`withWallClock` bound + a new `runAgent` `abortSignal` seam; a bounded **self-repair loop**, ≤2, feeds the real runtime error back) → **golden-eval** (3–7 auto-decomposed binary cases; judge = largest installed model clearing ~24B params, preferring a **different family than the generator**; a case passes only on a unanimous yes over 3 judge runs; **no qualifying judge ⇒ eval skipped, commit marked `verified: runs`** — degrade, never block, never an unconsented pull) → **commit** (index splice + `<name>.golden.json` + manifest upsert) at the earned `VerifiedLevel` (`behaves`/`runs`/`unverified`). A failed gate registers **nothing**; `verify.force` downgrades to an `unverified` commit. The manifest closes two prior gaps (the original need was never persisted; no usage tracking existed); `usage.ts` derives per-artifact use counts from every run's `spans.jsonl`, and `archive.ts` + `bun run archive [--prune]` reversibly archives idle artifacts that have a more-used near-duplicate (per-candidate consent). `build.verify`/`build.archive` telemetry spans; an informational reuse hint on chat's gap offers. In-repo **band-calibration eval** (`reuse.eval.test.ts`) instead of human-labeled judge calibration; plain cosine (tiny registry) instead of SimHash/ANN — deliberate scoping, recorded in §20. See `docs/architecture.md` §20.
...
- ~~**Per-agent/crew golden-eval**~~ — ✅ **shipped (Slice 20)**: 3–7 auto-decomposed binary cases per generated artifact (`<name>.golden.json`), judged unanimously 3× by the largest installed cross-family model; skipped (commit marked `verified: runs`) when no judge clears the ~24B bar — degrade, never block.
- ~~**Reuse / archive**~~ — ✅ **shipped (Slice 20)**: a pre-generation capability-signature cosine check against the per-registry `.generated.json` manifest (≥0.85 reuse · 0.75–0.85 offer · <0.75 generate) short-circuits near-duplicate generation; `bun run archive [--prune]` reversibly archives idle artifacts that have a more-used near-duplicate (per-candidate consent).
...
- **Repair-feedback re-stages the same proposal/IR** — the ≤2-attempt self-repair loop feeds the runtime error back, but v1's `stage(feedback)` rewrites the unchanged proposal/IR rather than routing the feedback into a targeted regeneration.
- **Judge *family* is a tag heuristic** — parsed from the model tag (no model-family registry exists in the repo); the cross-family preference is only as good as that parse.
