...
| **Voice INPUT (STT)** | 🟡 **re-scoped (Slice 29, 2026-07-07)** — voice *input* via a mature OSS STT library (`sherpa-onnx`), one approach for CLI + web. A hand-rolled full voice in/out + barge-in pipeline was built then **reset** (archived on branch `slice-29-voice-streaming-cli`) as the wrong fit for a terminal; rich interruptible voice → the web UI (browser-native AEC). See recommended-sequence item 20. | Audio |
...
20. **Voice INPUT via an OSS STT library** (Phase F, **Slice 29 — RE-SCOPED 2026-07-07**). Original scope ("voice in/out + streaming CLI") was **built and then reset**: a hand-rolled terminal pipeline (streaming CLI + `parakeet-mlx` streaming STT + `silero-vad` + `sounddevice` + Kokoro/`mlx-audio` TTS voice-out + barge-in + half-duplex) fought the OS (self-echo on speakers with no AEC, TTS reading markdown aloud) and was judged the wrong fit. That work is **archived, unmerged, on branch `slice-29-voice-streaming-cli`** (recoverable — the token-streaming CLI on it is independently reusable if ever wanted). New scope: a single mature **open-source STT library for voice INPUT**, usable in **both the CLI and the future web UI** — recommendation `sherpa-onnx` (streaming ASR + VAD, one codebase for Node + browser-WASM; fallback: a whisper.cpp/transformers.js Whisper pair or Moonshine). **Rich, interruptible ("human") voice belongs in the web UI (Slice 30)**, where the browser's `getUserMedia({audio:{echoCancellation:true}})` gives acoustic echo cancellation — the exact self-echo wall the terminal hit — essentially for free.
21. **TUI / local web UI** (Phase F, **Slice 30**) — the visual surface; reuses the pure `render-trace.ts` renderers. Uncensored mode's config option would live here (the axis itself already shipped default-ON in Slice 27, `AGENT_UNCENSORED=0` to disable — this is a future **UI surface** for that existing switch, not new plumbing).
...
| **34** | **Product surface** — a real chat UI / "second brain" (not CLI-only) | Biggest day-to-day usability gap; Slice 30's TUI covers run-history, not a primary chat surface | Slice 24 (daemon), Slice 30 (TUI groundwork) · **a persistence pass — verified 2026-07-05 chat.ts has ZERO cross-invocation memory (fresh `task` from argv every run, nothing read back) and `recall`/memory-RAG is wired into crews/workflows only, never chat's orchestrator; "second brain" needs both closed first, see [[reference-persistence-architecture-findings]]** · **web-validated 2026-07-05: Hermes Agent's thin-adapter multi-platform gateway (one process, per-platform adapters, circuit-breaker isolated, cross-platform session handoff) is concrete prior art for a chat surface; Flowise's one-node-per-MCP-server pattern + credential-stripped template marketplace + human-in-the-loop DAG node are concrete patterns for a future visual layer. See [[reference-2026-agent-tooling-trends-findings]]** |
