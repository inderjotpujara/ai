...
| **Graceful degradation + retries** — ✅ **shipped (Slice 21 — closes Phase A)** | If `uvx` / an MCP server / a model is down, **drop that agent and tell the user** instead of failing the whole CLI. `src/reliability/` — a three-lane error taxonomy (`Lane.Transient/RouteWorthy/Terminal`), retry with full-jitter backoff + `Retry-After` respect (Transient-only, no LLM-turn double-retry per D5), run/idle timeouts, a hand-rolled circuit breaker with a shared per-dependency registry, a failure-domain-aware model-degradation chain, and a user-facing `DegradationLedger` persisted to `run.dir/degradation.jsonl` — wired into delegation/workflow/crew/MCP/selector; the pre-existing provisioning + verified-build retry/timeout duplicates migrated onto it. **Live-verified on Ollama.** See [`docs/architecture.md`](architecture.md) §21. | Slices 2–3 |
