...
- **Quality-ranked selection** — now that Slice 6 populates the registry per-machine, a future slice can attach richer metadata (benchmark scores, eval results) so the selector ranks by quality-within-budget rather than pure size. (Metadata enrichment is the open part; ✅ the discovery signal it builds on shipped in Slice 6.)
- **Richer registry + discovery** — ✅ Delivered in Slice 6: the offline `buildRegistry` merge replaced the static bootstrap ladder with per-machine runtime discovery from Hugging Face (GGUF + MLX catalogs), installed-model tracking, and multi-quantization awareness.
