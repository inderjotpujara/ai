...
13. **Codex heavy-lifting backup** (Phase C, **Slice 22 — next**) — the last Phase-C item: tier-2 cloud delegate via `@openai/codex-sdk` (consent + cost design) for jobs too big for local models.
...
next slice is now **22** (Codex heavy-lifting backup), following Slice 21's
landing (graceful degradation + retries, shipped 2026-07-05), then 23–30 per
the committed sequence above. Slice 31 only becomes "next" once 30 ships; it
is the first item of *this* batch, not the project's current next step.
Slices 32–35 follow the same cycle afterward, in order, once 31 ships.
