...
- **The ComfyUI/Wan server lane (`comfy-lane.ts`'s `wanComfyStrategy`) is shape-only** — ComfyUI is not installed in this environment, so its node graph has never been exercised against a live server; the node ids/class names follow the published workflow shape but are unverified.
- **`runGenJob` (the `ExecMode`-aware degrade dispatcher) is not yet wired into `createGenerateTools`** — the live `generate_image`/`generate_speech`/`generate_video` tools call `runOneShotJob` directly against their fixed default strategy, so the one-shot↔server degrade path `runGenJob` implements (and is unit-tested against) has no caller in the actual product surface yet. A wiring gap, not a design gap.
...
- **`Capability.ImageGen/SpeechGen/VideoGen` are typed but unconsumed** — no `ModelDeclaration`/`ModelReq` uses them yet; generation is currently routed structurally by `MediaKind` to a fixed CLI strategy rather than through the hardware-fit selector. A future slice could route generation through the selector the same way vision already does.
