...
| **34** | **Product surface** — a real chat UI / "second brain" (not CLI-only) | Biggest day-to-day usability gap; Slice 29's TUI covers run-history, not a primary chat surface | Slice 24 (daemon), Slice 29 (TUI groundwork) · **a persistence pass — verified 2026-07-05 chat.ts has ZERO cross-invocation memory (fresh `task` from argv every run, nothing read back) and `recall`/memory-RAG is wired into crews/workflows only, never chat's orchestrator; "second brain" needs both closed first, see [[reference-persistence-architecture-findings]]** · **web-validated 2026-07-05: Hermes Agent's thin-adapter multi-platform gateway (one process, per-platform adapters, circuit-breaker isolated, cross-platform session handoff) is concrete prior art for a chat surface; Flowise's one-node-per-MCP-server pattern + credential-stripped template marketplace + human-in-the-loop DAG node are concrete patterns for a future visual layer. See [[reference-2026-agent-tooling-trends-findings]]** |
...
- ✅ **`Capability.{ImageGen,SpeechGen,VideoGen}` now drive a parallel gen-fit selector (Slice 28)** — generation model choice is hardware-adaptive: `generate/select.ts` `selectGenModel` ranks a per-modality `GenModelCandidate` ladder (`generate/catalog.ts`) largest-that-fits by footprint against the live budget, env-pin authoritative, consent-gated pull, graceful no-fit degrade, injecting the chosen repo via `GenOpts.model`. This is a **parallel** path (not the main `resolveModel`/`createModel` selector) *by design* — a gen engine spawns a CLI that writes a file rather than being warmed into a runtime as a `LanguageModel` (the enum values stay typed-but-not-`ModelDeclaration`-consumed for that reason; see `docs/architecture.md` §22). Live-verified: image→FLUX-4bit PNG, speech→Kokoro WAV, video graceful-degrade.
