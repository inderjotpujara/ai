...
| **Verified-build** | `src/verified-build/` | Verified artifact building (in progress): shared vocabulary + live config for verifying builder-generated artifacts (agents/crews/workflows) before commit. `types.ts` — verification levels (`VerifiedLevel`: behaves/runs/unverified), reuse decisions (`ReuseKind`, `ReuseDecision`), golden-case sets (`GoldenKind`, `GoldenCase`, `GoldenSet`), dry-run/eval outcomes (`DryRunResult`, `EvalResult`, `EvalCaseResult`), the capability manifest (`CapabilitySignature`, `ManifestEntry`, `Manifest`), the artifact kind (`ArtifactKind`), and the `VerificationResult` discriminated union (committed/reused/failed). `config.ts` — env-overridable live thresholds (dry-run budget, repair cap, reuse/offer similarity bands, judge min params, archive idle days, eval runs) via the `Number(process.env.X) \|\| DEFAULT` fallback-only pattern. Types + config only so far — later tasks add the dry-run/golden-eval/reuse pipeline on top | none yet (pure types + env-read config) |
