...
- **`spans.ts`** — the API: the **`ATTR`** key registry + helpers `withRunSpan` / `setRunOutcome` / `withDelegationSpan` / `recordModelSelect` / `withModelLoadSpan` / `recordEvict` / `recordGuardrailViolation` / `withWorkflowSpan` / `withStepSpan` / `annotateStep` (the last three back the workflow/DAG engine, §9). **AI-SDK spans (Slice 23 — `@ai-sdk/otel` extraction).** AI SDK v7 extracted OpenTelemetry out of core `ai`; passing `telemetry: {…}` (the renamed `experimental_telemetry`) to `generateText`/`streamText` no longer emits spans by itself. `src/telemetry/ai-sdk.ts`'s `ensureAiSdkTelemetry()` registers a `LegacyOpenTelemetry` integration once per process (`registerTelemetry`, guarded by a module boolean), given a `dynamicTracer` that re-resolves `trace.getTracer('ai')` on every `startSpan`/`startActiveSpan` call instead of capturing one — `@ai-sdk/otel` otherwise reads its tracer once, eagerly, which would freeze onto whichever provider was global at registration and stop following `run-router.ts`'s per-run global-tracer-provider swaps (and the InMemory provider tests install). `core/agent.ts` calls `ensureAiSdkTelemetry()` before every `generateText`/`streamText`. `LegacyOpenTelemetry` (not the newer `OpenTelemetry` class) preserves the v6 span shape — span name `ai.generateText`, the `ai.telemetry.functionId` attribute (`functionId = agent.name`), `ai.toolCall` / token spans — nested under our manual spans via the active context exactly as before; only the emission path changed, not the output, and spans still land in `runs/<id>/spans.jsonl` via the same `RunRoutingSpanProcessor`.
...
- **`@ai-sdk/mcp` v2 note (Slice 23).** Under `@ai-sdk/mcp@2`, the HTTP transport's `redirect` option now defaults to `'error'` (an SSRF hardening — v1 followed 3xx). `client.ts`'s `buildHttpTransportConfig` leaves it at that secure default, so a remote HTTP MCP server that responds with a 3xx redirect will fail to mount rather than be followed. The two built-in servers are stdio (unaffected); this only touches user-configured remote HTTP servers and is a **forward-item to revisit when Slice 24 opens remote access** — deliberately not weakened to `'follow'`, which would reintroduce the SSRF vector.
