...
- **`server.ts`** / **`sqlite-server.ts`** — the two in-repo servers: `read_file` (stdio), and `query` (SELECT-only) / `execute` (writes) / `schema` on `bun:sqlite` (the `sqlite` pack entry defaults to `data/agent.db`, which — like any relative path — must exist as a directory first; `bun:sqlite` does not create parent directories, so a bare clone needs `mkdir -p data` before its first `sqlite` mount, or `:memory:`).
...
because `data/` doesn't exist on this checkout (see above) — mounting
degraded per-entry as designed; the other three servers were unaffected.
